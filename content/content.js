function safeSendMessage(message) {
  if (!chrome.runtime?.id) {
    console.warn(
      "[BOJ Editor] Extension context invalidated. Please reload the page.",
    );
    return false;
  }
  try {
...
function extractProblemData() {
  ...
  const problemTitle =
    document.querySelector("#problem_title")?.textContent?.trim() ?? "";
  ...
  const username =
    document.querySelector("a.username")?.textContent?.trim() ?? null;
...
function injectCodeButton() {
  ...
  const buttonContainer = document.querySelector(
    ".btn-group.pull-right.problem-button",
  );
  ...
  btn.type = "button";
  btn.innerHTML = '<i class="fa fa-code"></i>Editor';
